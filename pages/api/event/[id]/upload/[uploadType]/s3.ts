...
export default async function(req, res) {
  const { id, uploadType, objectName, contentType } = req.query
  ...
  try {
    ...
    s3.getSignedUrl("putObject", s3Param, (err, data) => {
      ...
      res.status(200).json({
        success: true,
        signedUrl: data,
        uploadUrl: `https://${bucket}.s3.amazonaws.com/events/${id}/${uploadType}/${objectName}`
      })
    })
  } catch (err) {
    res.status(400)
